...
describe("Vuex: Pruebas en el auth-module", () => {
  ...
  test("Mutation: loginUser", () => {
    ...
    const payload = {
      user: { name: "Diego", email: "diego@yahoo.com" },
      idToken: "ABC-123",
    ...
    expect(user).toEqual({ name: "Diego", email: "diego@yahoo.com" });
  ...
  test("Getter: username currentState", () => {
    ...
    expect(store.getters["auth/username"]).toBe("Diego");
  });
  ...
  test("Actions: createUser signInUser - Crea el usuario", async () => {
    ...
    // Borrar el usuario
    await axios.post(
      `https://identitytoolkit.googleapis.com/v1/accounts:delete?key=AIzaSyBcTjbDEKAIJ-EtCpSyN9P7Y6FaXkO60ls`,
      { idToken }
    );
